 ...
 *
 * <p>
 * This is a <a href="../lang/doc-files/ValueBased.html">value-based</a> class; use of identity-sensitive operations (including reference equality ({@code ==}), identity hash code, or synchronization) on instances of {@code Optional} may have unpredictable results and should be avoided.
 *
 * @since 1.8
 */
public final class Optional<T> {
	 ...
	 *
	 * @apiNote Though it may be tempting to do so, avoid testing if an object is empty by comparing with {@code ==} against instances returned by {@code Option.empty()}. There is no guarantee that it is a singleton. Instead, use {@link #isPresent()}.
	 *
	 ...
	 * @return
	 */
	public <V> Supplier<V> ifPresent(V defaultVal, Function<T, V> fun) {
		return () -> {
			if (isPresent()) {
	 ...
	 * @return
	 */
	public <V> Consumer<V> ifPresent(BiConsumer<T, V> fun) {
		return (V val) -> {
			if (isPresent()) {
				fun.accept(get(), val);
			}
	...
	/**
	 * If a value is present, apply the provided mapping function to it, and if the result is non-null, return an {@code Optional} describing the result. Otherwise return an empty {@code Optional}.
	 *
	 * @apiNote This method supports post-processing on optional values, without the need to explicitly check for a return status. For example, the following code traverses a stream of file names, selects one that has not yet been processed, and then opens that file, returning an {@code Optional<FileInputStream>}:
	 *
	 *          <pre>
	 * {@code
	 ...
	 *          </pre>
	 *
	 *          Here, {@code findFirst} returns an {@code Optional<String>}, and then {@code map} returns an {@code Optional<FileInputStream>} for the desired file if one exists.
	 *
	...
	/**
	 * If a value is present, apply the provided {@code Optional}-bearing mapping function to it, return that result, otherwise return an empty {@code Optional}. This method is similar to {@link #map(Function)}, but the provided mapper is one whose result is already an {@code Optional}, and if invoked, {@code flatMap} does not wrap it with an additional {@code Optional}.
	 *
