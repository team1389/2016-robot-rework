...
/**
 * supplies a {@link RigidTransform2d} representing an estimate of the robot's
 * pose based on sensor input
 * 
 * @author amind
 */
public class RobotStateEstimator implements Supplier<RigidTransform2d> {
	 ...
	 * @param scrub
	 *            constant to deal with discrepancy between theoretical
	 *            effectivDiam and actual effective diam
	 */
	...
	/**
	 * estimates the robot's current pose based on the most recent pose
	 * observation and the distance travelled since then
	 * 
	 * @param left_encoder_delta_distance
	 *            the distance travelled by the left wheels since last state
	 *            observation
	 * @param right_encoder_delta_distance
	 *            the distance travelled by the right wheels since last state
	 *            observation
	 * @param current_gyro_angle
	 *            the angle rotated since last state observation
